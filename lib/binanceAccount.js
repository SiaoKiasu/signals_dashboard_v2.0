...
const ACCOUNT_SCALE_MULTIPLIER = Math.max(1, Number(process.env.BINANCE_ACCOUNT_SCALE_MULTIPLIER || 3));
...
function scaleAmount(value, digits = 8) {
  const n = parseNumber(value, null);
  if (!Number.isFinite(n)) return null;
  return Number((n * ACCOUNT_SCALE_MULTIPLIER).toFixed(digits));
}

function scaleDailyNetIncome(dailyNetIncome) {
  return (Array.isArray(dailyNetIncome) ? dailyNetIncome : []).map((item) => ({
    ...item,
    net_income: scaleAmount(item && item.net_income, 8) || 0,
  }));
}

function scaleTradeRecord(record, initialCapital) {
  if (!record || typeof record !== "object") return record;
  const scaledPnl = scaleAmount(record.pnl, 8);
  const scaledQty = scaleAmount(record.qty, 8);
  const scaledClosedQty = scaleAmount(record.closed_qty, 8);
  const scaledRemainingQty = scaleAmount(record.remaining_qty, 8);
  return {
    ...record,
    pnl: scaledPnl,
    return:
      Number.isFinite(initialCapital) && initialCapital > 0 && Number.isFinite(scaledPnl)
        ? Number((scaledPnl / initialCapital).toFixed(8))
        : null,
    commission: scaleAmount(record.commission, 8),
    qty: scaledQty,
    closed_qty: scaledClosedQty,
    remaining_qty: scaledRemainingQty,
    quote_qty: scaleAmount(record.quote_qty, 8),
  };
}

function equityByDateLookup(equityPoints) {
  const map = new Map();
  for (const item of Array.isArray(equityPoints) ? equityPoints : []) {
    const date = item && item.date ? String(item.date) : "";
    const equity = parseNumber(item && item.equity, null);
...
function applyTradeLeverage(trades, equityPoints) {
  const equityMap = equityByDateLookup(equityPoints);
  return (Array.isArray(trades) ? trades : []).map((item) => {
    const openDate = toDateKey(item && item.open_timestamp ? item.open_timestamp : item && item.timestamp);
    const openEquity = equityMap.get(openDate);
    const openAvgPrice = parseNumber(item && item.open_avg_price, null);
    const qty = parseNumber(item && item.qty, null);
    const leverage =
      Number.isFinite(openEquity) && openEquity > 0 && Number.isFinite(openAvgPrice) && Number.isFinite(qty)
        ? Number(((openAvgPrice * qty) / openEquity).toFixed(8))
        : null;
    return {
      ...item,
      leverage,
...
function getInitialCapital(currentWalletBalance, netIncomes, existingEquityDoc) {
  ...
  if (Number.isFinite(fromEnv) && fromEnv > 0) return scaleAmount(fromEnv, 8);
...
async function syncBinanceAccountData(db) {
  ...
  const currentWalletBalanceRaw =
    parseNumber(accountData && accountData.totalWalletBalance, null) ??
    parseNumber(accountData && accountData.totalMarginBalance, null) ??
    0;
  const currentWalletBalance = scaleAmount(currentWalletBalanceRaw, 8) || 0;
  const dailyNetIncome = scaleDailyNetIncome(buildDailyNetIncome(allIncomeRows));
  ...
  const rawTrades = userTrades.length ? aggregateUserTrades(userTrades, initialCapital) : normalizeTrades(realizedPnlRows, initialCapital);
  const trades = applyTradeLeverage(rawTrades, equityPoints);
