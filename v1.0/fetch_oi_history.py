...
API_URL = 'https://open-api-v4.coinglass.com/api/futures/open-interest/aggregated-history'
API_KEY = 'c156d56ebbbc40d9afe695264826efff'
SYMBOL = 'BTC'
INTERVAL = '1d'
OUTPUT_FILE = 'oi_history.html'
...
def parse_data(data):
    ...
    for item in data_list:
        ...
        oi_val = None
        for k in ['close', 'openInterest', 'value', 'oi', 'open_interest', 'amount', 'open']:
            if k in item:
                try:
                    oi_val = float(item[k])
                    break
...
def generate_html(dates, oi_values):
    """生成包含 ECharts 图表的 HTML"""
    
    dates_json = json.dumps(dates, ensure_ascii=False)
    values_json = json.dumps(oi_values, ensure_ascii=False)
    current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    data_count = len(dates)
    latest_oi = f"{oi_values[-1]:,.0f}" if oi_values else 'N/A'
    max_oi = f"{max(oi_values):,.0f}" if oi_values else 'N/A'
    min_oi = f"{min(oi_values):,.0f}" if oi_values else 'N/A'
...
<head>
  ...
  <title>OI History - __SYMBOL__</title>
...
<body>
  <div class="header">
    <h1>__SYMBOL__ Open Interest History</h1>
    <div class="subtitle">数据来源: Coinglass API | 更新于: __TIME__</div>
  </div>
  <div id="chart-container"></div>
  <div class="info">
    <strong>数据点数量:</strong> __COUNT__ 条 |
    <strong>最新 OI:</strong> __LATEST__ |
    <strong>最高 OI:</strong> __MAX__ |
    <strong>最低 OI:</strong> __MIN__
  </div>
  <script>
    ...
    const option = {
      ...
      tooltip: {
        ...
        formatter: function(params) {
          ...
          return '<div style="background: rgba(21,25,36,0.95); border:1px solid #2a2e39; border-radius:6px; padding:12px 16px;">' +
                 ...
                 '<span style="color:#787b86;">Open Interest</span>' +
                 '<span style="font-family:monospace; font-weight:600; font-size:14px; color:#fff;">' + item.value.toLocaleString() + '</span>' +
                 '</div></div>';
        }
      },
      legend: { data:['Open Interest'], textStyle:{ color:'#787b86', fontSize:12 }, top:10 },
      ...
      series: [
        { name:'Open Interest', type:'line', data:values, smooth:false, symbol:'none',
          lineStyle:{ color:'#2962ff', width:2 },
          areaStyle:{ color:new echarts.graphic.LinearGradient(0,0,0,1,[{ offset:0, color:'rgba(41,98,255,0.3)' },{ offset:1, color:'rgba(41,98,255,0)' }]) },
...
"""
    ...
    html_content = (html_template
                    ...
                    .replace("__LATEST__", latest_oi)
                    .replace("__MAX__", max_oi)
                    .replace("__MIN__", min_oi)
...
def main():
    ...
    dates, oi_values = parse_data(data)
    
    if not dates or not oi_values:
        print("数据解析失败，原始数据:")
    ...
    print(f"OI 范围: {min(oi_values):,.0f} 至 {max(oi_values):,.0f}")
    
    output_path = os.path.join(os.path.dirname(__file__), OUTPUT_FILE)
    print(f"正在生成 HTML 文件: {output_path}")
    
    html_content = generate_html(dates, oi_values)
